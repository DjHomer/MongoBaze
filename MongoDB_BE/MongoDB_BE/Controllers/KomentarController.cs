...
namespace MongoDB_BE.Controllers
{
...

    {
        ...
        [HttpGet]
        [Route("VratiKomentareZaBusPreduzece/{id}")]
        public ActionResult VratiKomentareZaZaBusPreduzece([FromRoute(Name = "id")] string id)
        {
            try
            {
                return new JsonResult(DataProvider.VratiKomentareZaBusPreduzece(id));
            }
            catch (Exception e)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, e.ToString());
        ...
        [HttpPost]
        [Route("KreirajKomentar/{busId}")]
        public ActionResult KreirajKomentar([FromRoute(Name = "busId")] string id, [FromBody] Komentar komentar)
        {
            try
            {
